...
namespace IG.Num
{
        ...
        ILockable
    {
        ...
        /// <param name="results">Reference to the object where request data for response evaluation is written to.</para
        // /// <returns>Object containing complete request data for response evaluation.</returns>
        protected override void CreateRequestThis(IVector parameters, ref VectorFunctionResults ret)
        {
            VectorFunctionResults request = new VectorFunctionResults();
            // Copy problem characteristics:
            request.NumParameters = this.NumParameters;
            request.NumFunctions = this.NumFunctions;
            // Copy request flags:
            request.ReqValues = this.ReqValues;
            request.ReqGradients = this.ReqGradients;
            request.ReqHessians = this.ReqHessians;
            // Copy parameters:
