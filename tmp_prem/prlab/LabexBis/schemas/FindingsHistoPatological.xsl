  ...
  <xsl:template match="/Findings">
    <html>
      ...
      <body>
        ...
        <H3>
          <xsl:text>ZAČASNE DIAGNOZA / MNENJE:</xsl:text>
        </H3>
        <xsl:value-of select="TemporaryOpinion"/>
        <br />
        ...
        <p>
          <b>
            <xsl:text>Zdravnik (začasna diagnoza): </xsl:text>
          </b>
          <xsl:value-of select="SignerPart"/>
          <xsl:text>,  </xsl:text>
          <xsl:value-of select="SignDatePart"/>
        </p>
        <br />
        <hr />
        <br />
        ...
        <H3>
          <xsl:text>MAKROSKOPSKI OPIS (nadaljevanje):</xsl:text>
        </H3>
        <xsl:value-of select="MacroDescription2"/>
        <br />

        <H3>
          <xsl:text>MIKROSKOPSKI OPIS (nadaljevanje):</xsl:text>
        </H3>
        <xsl:value-of select="MicroDescription2"/>
        <br />
        ...
        <p>
          <b>
            <xsl:text>Zdravnik (končna diagnoza): </xsl:text>
          </b>
          <xsl:value-of select="SignerPart"/>
          <xsl:text>,  </xsl:text>
          <xsl:value-of select="SignDatePart"/>
        ...
        <H3>
          <xsl:text>Dodatek k izvidu:</xsl:text>
        </H3>
        ...
        <p>
          <b>
            <xsl:text>Zdravnik (dodatek): </xsl:text>
          </b>
          <xsl:value-of select="SignerPart"/>
          <xsl:text>,  </xsl:text>
          <xsl:value-of select="SignDatePart"/>
